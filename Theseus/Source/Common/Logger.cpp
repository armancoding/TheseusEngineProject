...
VOID Logger::PrintLog(const WCHAR* fmt, ...)
{
	...
	{
		MessageBox(NULL, L"Unable to open lof file...", L"Log Error", MB_OK);
	}
...
VOID Logger::PrintDebugSeparator()
{
	...
	else { MessageBox(NULL, L"Unable to open log file...", L"Log Error", MB_OK); }
#endif
}
...
{
	Logger::PrintLog(L"--Starting MTrail\n");
