...
namespace Win32 {
	...
	VOID SubObject::RegisterNewClass()
	{
		...
		wcex.hbrBackground = (HBRUSH)(CreateSolidBrush(m_Theme ? LIGHTBACKGROUND : DARKBACKGROUND));	
		wcex.hIcon = m_hIcon;
		wcex.hIconSm = m_hIcon;
		wcex.lpszClassName = m_Class.c_str();
		wcex.lpszMenuName = nullptr;
		wcex.hInstance = HINSTANCE();
		wcex.lpfnWndProc = SetupMessaegeHandler;
		RegisterClassEx(&wcex);
	}

	LRESULT SubObject::SetupMessaegeHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		if (msg == WM_NCCREATE)
		{
